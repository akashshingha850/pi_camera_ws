 ...
 *
 * Image Processing Algorithm data serializer for rkisp1
 *
...
public:
		  ...
		  ControlSerializer *cs)
	{
		...
		std::vector<uint8_t> paramFormat;
		std::tie(paramFormat, std::ignore) =
			IPADataSerializer<uint32_t>::serialize(data.paramFormat);
		retData.insert(retData.end(), paramFormat.begin(), paramFormat.end());
		    ...
		    ControlSerializer *cs)
	{
		...
		if (dataSize < 4) {
			LOG(IPADataSerializer, Error)
				<< "Failed to deserialize " << "paramFormat"
				<< ": not enough data, expected "
				<< (4) << ", got " << (dataSize);
			return ret;
		}
		ret.paramFormat = IPADataSerializer<uint32_t>::deserialize(m, m + 4);
